...
const checkUserExists = async (req, res, next) => {
    // req hàm người dùng gửi lên, res trả về cho người dùng , next chuyển qua app.js hàm báo lỗi
    try {
...
const createUser = async (req, res, next) => {
    ...
    try {
        ...
        var MaKH;
        if (!check) {
            //nếu người dùng không tồn tại thì tạo acc mới
            // await User.findOne({})
            //     .sort({ MaKH: "desc" })
            //     .then((last) => {
            ...
            const token = jwt.sign(
                //tạo accsec token để lưu thông tin người dùng
...
const loginUser = async (req, res, next) => {
    try {
        ...
        if (user && bcrypt.compareSync(req.body.MatKhau, user.MatKhau)) {
            ...
            //kiểm tra có lỗi thì xóa cookie
            return res
